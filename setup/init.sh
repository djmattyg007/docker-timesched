...
if [[ -n "${TIMESCHED_HTTP_SERVERNAME}" ]]; then
    ...
    sed -e "s/___SERVER_NAME___/${TIMESCHED_HTTP_SERVERNAME}/" /etc/timesched/nginx/http.ngx > /etc/nginx/sites-available/http.ngx
else
    rm -f /etc/nginx/sites-available/http.ngx
fi
if [[ -n "${TIMESCHED_HTTPS_SERVERNAME}" ]]; then
    echo "[info] Found HTTPS servername, adding HTTPS configuration to nginx"
    sed -e "s/___SERVER_NAME___/${TIMESCHED_HTTPS_SERVERNAME}/" /etc/timesched/nginx/https.ngx > /etc/nginx/sites-available/https.ngx
else
    rm -f /etc/nginx/sites-available/https.ngx
fi
